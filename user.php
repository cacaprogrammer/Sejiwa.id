...
// ===== AUTO-CREATE: tabel tb_notifikasi =====
$conn->query("
...
if (isset($_GET['hapus'])) {
    ...
    // 1. Validasi: Pastikan user ada dan bukan admin
    $stmt_cek = $conn->prepare("SELECT role, username FROM tb_user WHERE id = ?");
    $stmt_cek->bind_param("i", $hapus_id);
    $stmt_cek->execute();
    $result_cek = $stmt_cek->get_result();
    $user_data  = $result_cek->fetch_assoc();

    if (!$user_data) {
        echo "<script>window.location.href='dashboardAdmin.php?page=user&msg=user_tidak_ada';</script>";
        exit();
    }
    ...
    try {
        // 3. Hapus semua data terkait user (urutan penting — child dulu baru parent)
        $conn->query("DELETE FROM tb_notifikasi WHERE user_id = $hapus_id");
        $conn->query("DELETE FROM tb_favorit    WHERE user_id = $hapus_id");
        $conn->query("DELETE FROM tb_history    WHERE user_id = $hapus_id");
        $conn->query("DELETE FROM tb_ulasan     WHERE user_id = $hapus_id");
        $conn->query("DELETE FROM tb_artikel    WHERE created_by = $hapus_id");

        // 4. Hapus user
        $stmt_delete = $conn->prepare("DELETE FROM tb_user WHERE id = ? AND role != 'admin'");
        ...
        echo "<script>window.location.href='dashboardAdmin.php?page=user&msg=error_hapus';</script>";
    }
    exit();
}
...
// Ambil error tambah dari session
...
// ===== Fungsi khusus URL hapus =====
function hapusUrl($id) {
    $base = $_GET;
    unset($base['hapus'], $base['msg']);
    $base['page']  = 'user';
    $base['hapus'] = (int)$id;
...
$notif_map = [
    ...
    'error_hapus'    => ['error',   '⛔ Gagal menghapus user, mungkin ada data terkait.'],
    'user_tidak_ada' => ['warning', '⚠️ User tidak ditemukan di sistem.'],
];
$pesan = $tipe = '';
...
:root {
    ...
    --yellow-bg:#FFFBEB; --yellow-tx:#92400E;
...
.up-notif.warning{background:var(--yellow-bg);color:var(--yellow-tx);border-left:4px solid #F59E0B}
...
<div class="up-notif <?= $tipe ?>" id="up-notif">
    <div class="up-notif-ico">
        <i class='bx <?= $tipe === "sukses" ? "bx-check" : ($tipe === "error" ? "bx-x" : "bx-info-circle") ?>' style="font-size:17px"></i>
    </div>
    <span><?= $pesan ?></span>
        ...
        <form method="post" action="dashboardAdmin.php?page=user" id="up-form-tambah">
            ...
            <div class="up-form-row full" style="margin-bottom:12px">
                <div class="up-form-group">
                    ...
                    <input type="text" name="t_nama"
                           class="up-form-input <?= !empty($err_tambah)?'err':'' ?>"
