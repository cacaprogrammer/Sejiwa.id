...
// Ambil data artikel berdasarkan ID dari database
$artikel = getArtikelById($conn, $id_artikel);
...
// Ambil data User dari session (default id 1 jika belum login)
$id_user = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 1; 

// Jalankan pencatatan riwayat dan view count
catatHistory($conn, $id_user, $id_artikel);
tambahViewCount($conn, $id_artikel);

// Ambil data rating ringkasan
$rating = getRatingRingkasan($conn, $id_artikel);
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Isi <?= htmlspecialchars($artikel['judul']); ?></title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .main-title { font-size: 40px; font-weight: 700; color: #000; line-height: 1; margin: 0; padding-bottom: 35px; position: relative; top: 40px; text-transform: uppercase; }
...
<body>
    ...
    <div class="content-container">
        <div class="article-title-bar">
            ...
            <span class="site-name-right">sejiwa.id</span>
        </div>
        <hr class="separator">
        ...
        <div class="article-content">
            <div class="left-column">
                <div class="article-card-wrapper">
                    <div class="image-box">
                        <img src="<?= getThumbnail($artikel['thumbnail']); ?>" alt="<?= htmlspecialchars($artikel['judul']); ?>" class="main-article-image">
                    </div>
    ...
    <script>
        // ============================================================
        // DATA ARTIKEL DINAMIS MENGIKUTI DATABASE PHP
        // ============================================================
        const ARTIKEL_DATA = {
            id: '<?= $artikel['id']; ?>',
            judul: '<?= addslashes($artikel['judul']); ?>',
            gambar: '<?= getThumbnail($artikel['thumbnail']); ?>',
            link: 'isiArtikelPertempuranSurabaya.php?id=<?= $artikel['id']; ?>',
            ratingKey: 'sejiwa_rating_<?= $artikel['id']; ?>'
        };
        ...
        // Inisialisasi awal ikon bookmark saat halaman dimuat
        updateBookmarkIcon();
    </script>
</body>
</html>