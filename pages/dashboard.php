...
// Ambil semua ulasan untuk modal (join dengan user dan artikel)
$ulasan_list = $conn->query("
    SELECT u.id, u.rating, u.komentar, u.created_at,
           usr.nama_lengkap, usr.username,
           a.judul AS judul_artikel, a.kategori
    FROM tb_ulasan u
    LEFT JOIN tb_user    usr ON usr.id = u.user_id
    LEFT JOIN tb_artikel a   ON a.id   = u.artikel_id
...
<style>
  ...
  .page-header .header-badge {
    ...
    background: var(--cream); border: 1px solid rgba(74,44,24,0.15);
    color: var(--brown); font-size: 0.75rem; font-weight: 500;
    padding: 0.25rem 0.75rem; border-radius: 999px; margin-bottom: 0.5rem;
  }

  .stat-card {
    background: #fff; border-radius: 16px; padding: 1.25rem 1.5rem;
    box-shadow: var(--shadow-sm); border: 1px solid rgba(74,44,24,0.06);
  ...
  .stat-value { font-family: 'Montserrat', sans-serif; font-size: 2rem; font-weight: 800; color: var(--dark-brown); line-height: 1; }
  .stat-label { font-size: 0.8rem; color: var(--text-muted); margin-top: 0.3rem; font-weight: 400; }
  .stat-trend {
  ...
  .visitor-card h2 {
    ...
    text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.75; margin-bottom: 0.25rem;
  }
  .visitor-total { font-family: 'Montserrat', sans-serif; font-size: 3.5rem; font-weight: 800; line-height: 1; margin: 0.25rem 0; }
  .visitor-sub { font-size: 0.8rem; opacity: 0.65; margin-bottom: 1.25rem; }

  .mini-bars { display: flex; align-items: flex-end; gap: 5px; height: 52px; margin-top: 0.5rem; }
  .mini-bar { flex: 1; border-radius: 4px 4px 0 0; background: rgba(255,255,255,0.3); transition: background 0.2s; position: relative; cursor: default; }
  .mini-bar.active { background: var(--orange-brown); }
  .mini-bar:hover { background: rgba(255,255,255,0.5); }
  .mini-bar .bar-tip { display: none; position: absolute; bottom: 110%; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); color: #fff; font-size: 0.6rem; padding: 2px 5px; border-radius: 4px; white-space: nowrap; }
  .mini-bar:hover .bar-tip { display: block; }
  .bar-labels { display: flex; gap: 5px; margin-top: 4px; }
  .bar-labels span { flex: 1; text-align: center; font-size: 0.55rem; opacity: 0.5; }

  .popular-card { background: #fff; border-radius: 16px; padding: 1.5rem; box-shadow: var(--shadow-sm); border: 1px solid rgba(74,44,24,0.06); }
  .card-title { font-family: 'Montserrat', sans-serif; font-size: 0.95rem; font-weight: 700; color: var(--dark-brown); margin-bottom: 1.1rem; display: flex; align-items: center; justify-content: space-between; }
  .card-title .see-all { font-size: 0.72rem; font-weight: 500; color: var(--orange-brown); text-decoration: none; opacity: 0.85; transition: opacity 0.15s; }
  .card-title .see-all:hover { opacity: 1; text-decoration: underline; }

  .pop-item { display: flex; align-items: center; gap: 0.875rem; padding: 0.65rem 0.5rem; border-bottom: 1px solid rgba(74,44,24,0.06); transition: background 0.15s; border-radius: 8px; margin: 0 -0.5rem; }
  .pop-item:last-child { border-bottom: none; }
  .pop-item:hover { background: rgba(248,248,220,0.6); }
  .rank-badge { width: 28px; height: 28px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-family: 'Montserrat', sans-serif; font-size: 0.8rem; font-weight: 700; flex-shrink: 0; }
  ...
  .pop-thumb { width: 44px; height: 44px; border-radius: 8px; object-fit: cover; background: var(--cream); flex-shrink: 0; border: 1px solid rgba(74,44,24,0.08); }
  .pop-thumb-placeholder { width: 44px; height: 44px; border-radius: 8px; background: linear-gradient(135deg, var(--cream), rgba(173,141,119,0.2)); flex-shrink: 0; display: flex; align-items: center; justify-content: center; color: var(--soft-brown); border: 1px solid rgba(74,44,24,0.08); }
  ...
  .category-card { background: linear-gradient(145deg, var(--soft-brown), #BFA090); border-radius: 16px; padding: 1.5rem; color: #fff; box-shadow: var(--shadow-md); }
  .progress-ring-wrap { display: flex; align-items: center; justify-content: center; gap: 1.5rem; margin-top: 1rem; }
  ...
  .ring-label { position: absolute; font-family: 'Montserrat', sans-serif; font-size: 1.4rem; font-weight: 800; color: #fff; }
  .legend-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
  .legend-item { display: flex; align-items: center; gap: 0.5rem; font-size: 0.8rem; }

  .latest-card { background: #fff; border-radius: 16px; padding: 1.5rem; box-shadow: var(--shadow-sm); border: 1px solid rgba(74,44,24,0.06); }
  .latest-item { display: flex; align-items: flex-start; gap: 0.875rem; padding: 0.65rem 0.5rem; border-bottom: 1px solid rgba(74,44,24,0.06); transition: background 0.15s; border-radius: 8px; margin: 0 -0.5rem; }
  .latest-item:last-child { border-bottom: none; }
  .latest-item:hover { background: rgba(248,248,220,0.6); }
  .kat-badge { display: inline-block; font-size: 0.65rem; font-weight: 600; padding: 0.2rem 0.55rem; border-radius: 6px; text-transform: uppercase; letter-spacing: 0.04em; flex-shrink: 0; margin-top: 0.1rem; }
  ...
  /* ===== ULASAN CARD — bisa diklik ===== */
  .ulasan-card {
    background: linear-gradient(145deg, var(--orange-brown), #C47830);
    border-radius: 16px; padding: 1.25rem 1.5rem; color: #fff;
    box-shadow: var(--shadow-md);
    transition: transform 0.2s ease, box-shadow 0.2s ease;
    cursor: pointer;
    user-select: none;
  }
  .ulasan-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg); }
  .ulasan-card:active { transform: translateY(0); }
  .ulasan-value { font-family: 'Montserrat', sans-serif; font-size: 2.2rem; font-weight: 800; line-height: 1; }
  .ulasan-hint { font-size: 0.7rem; opacity: 0.75; margin-top: 0.4rem; display: flex; align-items: center; gap: 0.3rem; }
  ...
  .modal-header h3 { font-family: 'Montserrat', sans-serif; font-size: 1rem; font-weight: 700; }
  .modal-header p  { font-size: 0.75rem; opacity: 0.7; margin-top: 0.1rem; }
  .modal-close {
  ...
  .modal-search {
    ...
    border-bottom: 1px solid rgba(74,44,24,0.07);
    flex-shrink: 0;
  }
  .modal-search input {
    width: 100%; padding: 0.5rem 0.85rem;
    border: 1px solid rgba(74,44,24,0.18); border-radius: 8px;
    font-size: 0.85rem; outline: none; font-family: 'Roboto', sans-serif;
    transition: border-color 0.2s;
  }
  .modal-search input:focus { border-color: var(--orange-brown); box-shadow: 0 0 0 2px rgba(217,139,62,0.15); }

  .modal-body { overflow-y: auto; flex: 1; padding: 0.5rem 1.5rem 1.5rem; }
  .modal-body::-webkit-scrollbar { width: 6px; }
  .modal-body::-webkit-scrollbar-thumb { background: #ddd; border-radius: 99px; }

  /* Ulasan item di modal */
  .uitem {
    display: flex; gap: 0.875rem; padding: 0.95rem 0;
  ...
  .uitem:hover { background: rgba(248,248,220,0.5); }
  ...
  .uitem-top { display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; }
  .uitem-nama { font-size: 0.85rem; font-weight: 600; color: var(--text-main); }
  .uitem-stars { color: #F59E0B; font-size: 0.8rem; letter-spacing: 1px; }
  .uitem-artikel {
    font-size: 0.68rem; background: rgba(74,44,24,0.08);
    color: var(--brown); padding: 0.1rem 0.5rem; border-radius: 6px;
    max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  }
  .uitem-komentar { font-size: 0.82rem; color: #555; margin-top: 0.35rem; line-height: 1.45; }
  .uitem-date { font-size: 0.68rem; color: var(--text-muted); margin-top: 0.25rem; }
  ...
  .filter-btn {
    ...
    border-radius: 999px; border: 1px solid rgba(74,44,24,0.18);
    background: #fff; color: var(--brown); cursor: pointer;
    transition: all 0.15s;
  }
  .filter-btn.active, .filter-btn:hover {
    background: var(--dark-brown); color: #fff; border-color: var(--dark-brown);
  }
...
<div class="modal-overlay" id="modalUlasan">
  <div class="modal-box">
    ...
    <div class="modal-search">
      <input type="text" id="searchUlasan" placeholder="Cari nama pengguna, artikel, atau komentar...">
    </div>
...
<div class="dashboard-wrap">
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="stat-card c3">
      ...
      <span class="stat-trend trend-neu">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 7h10M7 12h10M7 17h4" /></svg>
        Kategori
      </span>
    </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
    ...
    <div class="lg:col-span-3 flex flex-col gap-5">
      ...
      <div class="visitor-card">
        ...
        <div class="bar-labels">
          <?php foreach ($bar_labels as $bl): ?><span><?= $bl ?></span><?php endforeach; ?>
        </div>
      ...
      <div class="popular-card">
        <div class="card-title">
          ...
          <a href="dashboardAdmin.php?page=artikel" class="see-all">Lihat Semua →</a>
        </div>
        ...
        <p style="font-size:0.85rem; color:var(--text-muted); text-align:center; padding:1.5rem 0;">Belum ada data artikel.</p>
        <?php endif; ?>
      </div>
    ...
    <div class="lg:col-span-2 flex flex-col gap-5">
      ...
      <!-- ===== ULASAN CARD — BISA DIKLIK ===== -->
      <div class="ulasan-card" id="btnBukaUlasan" onclick="bukaModalUlasan()" title="Klik untuk lihat semua ulasan">
      ...
      <div class="category-card">
        ...
        $radius = 52; $circ = round(2 * 3.14159 * $radius, 2);
        ...
        <div class="progress-ring-wrap">
          <div style="position:relative; width:120px; height:120px; flex-shrink:0;">
            <svg width="120" height="120" class="ring-svg" viewBox="0 0 120 120">
              ...
              <circle class="ring-fill ring-fill-dark" cx="60" cy="60" r="<?= $radius ?>" stroke-dasharray="<?= $dash_s ?> <?= $circ - $dash_s ?>" stroke-dashoffset="0"/>
              <circle class="ring-fill ring-fill-light" cx="60" cy="60" r="<?= $radius ?>" stroke-dasharray="<?= $circ - $dash_s ?> <?= $dash_s ?>" stroke-dashoffset="-<?= $dash_s ?>"/>
            </svg>
          ...
          <div style="display:flex; flex-direction:column; gap:0.75rem;">
            <div class="legend-item">
              ...
              <div><p style="font-size:0.8rem; font-weight:600;">Sejarah</p><p style="font-size:0.7rem; opacity:0.75;"><?= $total_sejarah ?> artikel · <?= $pct_sejarah ?>%</p></div>
            </div>
            <div class="legend-item">
              <span class="legend-dot" style="background:rgba(255,255,255,0.85); border:1px solid rgba(0,0,0,0.1);"></span>
              <div><p style="font-size:0.8rem; font-weight:600;">Biografi</p><p style="font-size:0.7rem; opacity:0.75;"><?= $total_biografi ?> artikel · <?= $pct_biografi ?>%</p></div>
            </div>
          </div>
        </div>
        <div style="margin-top:1.25rem; display:flex; flex-direction:column; gap:0.6rem;">
          <div>
            <div style="display:flex; justify-content:space-between; margin-bottom:0.3rem;"><span style="font-size:0.72rem; opacity:0.8;">Sejarah</span><span style="font-size:0.72rem; font-weight:700;"><?= $pct_sejarah ?>%</span></div>
            <div style="height:6px; background:rgba(255,255,255,0.2); border-radius:99px; overflow:hidden;"><div style="width:<?= $pct_sejarah ?>%; height:100%; background:var(--dark-brown); border-radius:99px;"></div></div>
          </div>
          <div>
            <div style="display:flex; justify-content:space-between; margin-bottom:0.3rem;"><span style="font-size:0.72rem; opacity:0.8;">Biografi</span><span style="font-size:0.72rem; font-weight:700;"><?= $pct_biografi ?>%</span></div>
            <div style="height:6px; background:rgba(255,255,255,0.2); border-radius:99px; overflow:hidden;"><div style="width:<?= $pct_biografi ?>%; height:100%; background:rgba(255,255,255,0.75); border-radius:99px;"></div></div>
          </div>
        </div>
      </div>
      ...
      <div class="latest-card">
        ...
        <div class="latest-item">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="latest-date">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        ...
        <p style="font-size:0.85rem; color:var(--text-muted); text-align:center; padding:1.5rem 0;">Belum ada artikel.</p>
        <?php endif; ?>
      </div>
...
<script>
// ===== DATA ULASAN (dari PHP ke JS) =====
const SEMUA_ULASAN = <?php
  $ulasan_arr = [];
  if ($ulasan_list && $ulasan_list->num_rows > 0) {
    while ($u = $ulasan_list->fetch_assoc()) {
      $ulasan_arr[] = [
        'nama'    => $u['nama_lengkap'] ?: $u['username'] ?: 'Anonim',
        'inisial' => strtoupper(substr($u['nama_lengkap'] ?: $u['username'] ?: 'A', 0, 1)),
        'rating'  => (int)$u['rating'],
        'komentar'=> $u['komentar'],
        'artikel' => $u['judul_artikel'] ?: '—',
        'kategori'=> $u['kategori'] ?: '',
        'tanggal' => date('d M Y, H:i', strtotime($u['created_at']))
      ];
    }
  }
  echo json_encode($ulasan_arr, JSON_UNESCAPED_UNICODE);
?>;

let filterAktif = 'semua';
let querySearch  = '';

function bintangHTML(n) {
...
function renderModal() {
  ...
  let data = SEMUA_ULASAN;

  // Filter rating
  if (filterAktif !== 'semua') {
    data = data.filter(u => u.rating === parseInt(filterAktif));
  }
  ...
  if (querySearch.trim()) {
    ...
    data = data.filter(u =>
      ...
      u.komentar.toLowerCase().includes(q) ||
      u.artikel.toLowerCase().includes(q)
    );
  }

  subtitle.textContent = data.length + ' ulasan ditemukan';

  if (data.length === 0) {
    body.innerHTML = '<div class="modal-empty"><p style="font-size:2rem;">💬</p><p style="margin-top:0.5rem;">Tidak ada ulasan yang cocok.</p></div>';
    return;
  }

  body.innerHTML = data.map(u => `
    <div class="uitem">
      <div class="uitem-avatar">${u.inisial}</div>
      <div class="uitem-body">
        <div class="uitem-top">
          <span class="uitem-nama">${u.nama}</span>
          <span class="uitem-stars">${bintangHTML(u.rating)}</span>
          <span class="uitem-artikel" title="${u.artikel}">${u.artikel}</span>
        </div>
        <p class="uitem-komentar">${u.komentar}</p>
        <p class="uitem-date">🕐 ${u.tanggal}</p>
      </div>
    </div>
  `).join('');
}
...
function bukaModalUlasan() {
  filterAktif = 'semua';
  querySearch  = '';
  document.getElementById('searchUlasan').value = '';
  document.querySelectorAll('.filter-btn').forEach(b => b.classList.toggle('active', b.dataset.filter === 'semua'));
  renderModal();
  document.getElementById('modalUlasan').classList.add('show');
  document.body.style.overflow = 'hidden';
}

function tutupModal() {
  document.getElementById('modalUlasan').classList.remove('show');
